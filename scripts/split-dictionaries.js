...
function extractSection(dictionary, sectionKeys) {
  ...
  sectionKeys.forEach((key) => {
    if (dictionary[key]) {
      ...
      console.warn(`⚠️  Key "${key}" not found in dictionary`);
    }
  });
...
function writeSectionFile(locale, sectionName, data) {
  ...
  console.log(`✅ Created: ${locale}/${sectionName}.json (${sizeKB} KB)`);
...
function splitDictionary(locale) {
  console.log(`\n📦 Processing ${locale.toUpperCase()} dictionary...`);
...
function backupOriginals() {
  ...
  LOCALES.forEach((locale) => {
    ...
    if (fs.existsSync(original)) {
      ...
      console.log(`✅ Backed up: ${locale}.json → ${locale}.json.backup`);
    }
  });
}
 ...
 */
function main() {
  console.log('🚀 Dictionary Splitter v1.0.0');
  console.log('================================\n');
  ...
  try {
    ...
    console.log('\n✨ Dictionary splitting completed successfully!');
    console.log('\n📁 New structure:');
    ...
    LOCALES.forEach((locale) => {
      ...
      Object.keys(SECTION_MAPPING).forEach((section, idx, arr) => {
        ...
        console.log(`   │   ${isLast ? '└' : '├'}── ${section}.json`);
      });
    });
