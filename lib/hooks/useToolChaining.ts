...
} from '@/lib/stores/toolChainStore';
...
export function useToolChaining({
  ...
  const currentTool = tools.find((tool) => tool.id === toolSlug);
  ...
  const chainToTool = useCallback(
    (targetToolSlug: string, data?: string) => {
  ...
  const getChainContext = useCallback(() => {
    const currentStepIndex = currentChain.findIndex(
      (step) => step.toolSlug === toolSlug
    );
    const previousSteps =
      currentStepIndex > -1
        ? currentChain.slice(0, currentStepIndex)
        : currentChain;
    const nextSteps =
      currentStepIndex > -1 ? currentChain.slice(currentStepIndex + 1) : [];
