...
import {
  ...
  isValidBase64,
  downloadBlob,
} from '@/lib/tools/base64-to-jpg';
import { useCopy } from '@/lib/hooks/useCopy';
import { useToolTracking } from '@/lib/analytics/hooks/useToolTracking';
import { useScrollToResult } from '@/lib/hooks/useScrollToResult';

interface Base64ToJpgToolProps {
  categoryColor: string;
}

export default function Base64ToJpgTool({
  categoryColor,
}: Base64ToJpgToolProps) {
  const [input, setInput] = useState('');
  const [error, setError] = useState<string | null>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [result, setResult] = useState<Base64ToJpgResult | null>(null);
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      const conversionResult = base64ToJpg(input, {
        fileName: fileName || 'image.jpg',
      });

      setResult(conversionResult);

      if (conversionResult.success && conversionResult.jpgBlob) {
        // Create preview URL
        const url = URL.createObjectURL(conversionResult.jpgBlob);
        setPreviewUrl(url);
  ...
  const handleDownload = useCallback(() => {
    if (result?.jpgBlob && result.fileName) {
      downloadBlob(result.jpgBlob, result.fileName);
    }
  }, [result]);
      ...
      <div className="space-y-4">
        <div>
          ...
          <textarea
            ...
            placeholder="Paste your Base64 encoded JPEG data here..."
        ...
        >
          ...
          {showPreview && previewUrl && (
            <div className="space-y-2">
              ...
              <div className="flex justify-center rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-600 dark:bg-gray-700/50">
                ...
                <img
                  ...
                  src={previewUrl}
                  alt="JPEG Preview"
                  className="max-h-[400px] max-w-full rounded border border-gray-300 dark:border-gray-600"
