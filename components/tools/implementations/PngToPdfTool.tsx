...
export default function PngToPdfTool({ categoryColor }: PngToPdfToolProps) {
  ...
  const { trackCustom, trackError } = useToolTracking('png-to-pdf');
  ...
  useEffect(() => {
    if (!fileName) {
      ...
      setFileName(`png_to_pdf_${timestamp}.pdf`);
    }
  }, [fileName]);
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        >
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            PNG files only (up to 10MB per file) - Transparency supported
          </p>
          <input
            ref={fileInputRef}
            type="file"
