...
export default function JpgToPdfTool({ categoryColor }: JpgToPdfToolProps) {
  ...
  const { trackCustom, trackError } = useToolTracking('jpg-to-pdf');
  ...
  useEffect(() => {
    if (!fileName) {
      ...
      setFileName(`jpg_to_pdf_${timestamp}.pdf`);
    }
  }, [fileName]);
  ...
  const handleFileSelect = useCallback(async (files: FileList | null) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (
        ...
        setError(`${file.name} is not a JPG/JPEG file`);
        continue;
      }
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-800">
        ...
        >
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            JPG, JPEG files only (up to 10MB per file)
          </p>
          <input
            ref={fileInputRef}
            type="file"
        ...
        {images.length > 0 && (
          <div className="mt-6 space-y-3">
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300">
              Selected JPG Images ({images.length})
            </h3>
